...
export function ConformityTable<Field extends string>({ rows, values }: ConformityTableProps<Field>) {
  return (
    <>
      ...
      {rows.map((row, index) => {
        if (row.hideIfNotYes && values?.[row.field] !== ControlCheck.YES) return null

        return (
          <React.Fragment key={row.field}>
            {(row.dividerBefore || (row.sectionLabel && index > 0)) && (
              <div style={{ borderTop: `1px solid ${THEME.color.lightGray}`, margin: '0.5rem 0' }} />
            )}
            {row.sectionLabel && (
              <Stack.Item style={{ padding: '0.5rem 1rem 0' }}>
                <Label style={{ margin: 0 }}>{row.sectionLabel}</Label>
              </Stack.Item>
            )}
            <Stack direction="row" alignItems="center">
