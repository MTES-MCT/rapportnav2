...
const MissionGeneralInformationServiceUlam: FC<MissionGeneralInformationServiceUlamProps> = ({ name, fieldFormik, administrations }) => {
  const [initialValues, setInitialValues] = useState({ interMinisterialServices: [{ administrationId: undefined, controlUnitId: undefined }] })

  useEffect(() => {
    if (!fieldFormik.field?.value || fieldFormik.field.value.length === 0) return
    setInitialValues({ interMinisterialServices: fieldFormik.field.value })
  }, [fieldFormik])

  const handleSubmit = (input: Record<string, any>) => {

    const newValue = input.interMinisterialServices.filter((t: InterMinisterialService) => t.administrationId !== undefined && t.controlUnitId !== undefined)
              ...
              <Stack direction="column" style={{ width: '100%' }}>
                <Stack.Item style={{ width: '100%' }}>
                  {values.interMinisterialServices?.map((service, index) => {
                    ...
                    const unitOptions = selectedAdmin?.controlUnits?.map(unit => ({ value: unit.id, label: unit.name })) || []

                    return (
                      <Stack direction="row" alignItems="flex-start" key={index} style={{ width: '100%', marginTop: 6 }}>
                          ...
                          <Stack direction="column" style={{ width: '100%' }}>
                            <Stack.Item style={{ width: '100%' }}>
                              <FormikSelect
                                ...
                                isRequired
                                searchable
                                options={administrations ? administrations.map(admin => ({ value: admin.id, label: admin.name })) : []}
                ...
                <Stack.Item style={{ width: '100%', marginTop: 16 }}>
                  <Button
                    ...
                    disabled={values.interMinisterialServices.some(service => !service.administrationId || !service.controlUnitId)}
                  >
