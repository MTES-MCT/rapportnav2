...
import { useCrewAbsenceForm, MissionCrewAbsenceInitialInput } from '../use-crew-absence-form.tsx'
...
describe('useCrewAbsenceForm', () => {
  ...
  describe('EMPTY_ABSENCE constant', () => {
    it('returns EMPTY_ABSENCE with all fields undefined except isAbsentFullMission', () => {
      const { result } = renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))
  ...
  describe('fromFieldValueToInput transformation', () => {
    it('transforms MissionCrewAbsence to input format with dates array', () => {
      renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))
  ...
  describe('fromInputToFieldValue transformation', () => {
    ...
    it('calls postprocessDateFromPicker with endOfDay for startDate (dates[0])', () => {
      renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))
      ...
      expect(mockPostprocessDateFromPicker).toHaveBeenCalledWith(endOfDay(startDate))
    })

    it('calls postprocessDateFromPicker with startOfDay for endDate (dates[1])', () => {
      renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))
      ...
      expect(mockPostprocessDateFromPicker).toHaveBeenCalledWith(startOfDay(endDate))
    })
    ...
    it('handles undefined dates in array', () => {
      ...
      expect(mockPostprocessDateFromPicker).toHaveBeenCalledTimes(2)
  ...
  describe('isAbsentFullMission computation in fromInputToFieldValue', () => {
    it('sets isAbsentFullMission to true when absence dates match mission dates', () => {
      ...
      renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik(), 'mission-123'))
  ...
  describe('useAbstractFormikSubForm integration', () => {
    ...
    it('does not return errors from useAbstractFormikSubForm', () => {
      const { result } = renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))

      expect(result.current).not.toHaveProperty('errors')
    })
  })
  ...
  describe('validationSchema', () => {
    ...
    it('validationSchema is memoized (same reference on re-render)', () => {
      const { result, rerender } = renderHook(() => useCrewAbsenceForm('crew.0.absences.0', createFieldFormik()))
