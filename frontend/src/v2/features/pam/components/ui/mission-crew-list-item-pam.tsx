...
import { Accent, Checkbox, Icon, IconButton, Size, THEME } from '@mtes-mct/monitor-ui'
...
const StandardCrewMember: React.FC<CrewListProps> = ({
  ...
  return (
    <FlexboxGrid key={`${name}-${index}`} align={'middle'}>
      <FlexboxGrid.Item colspan={1} style={{ paddingRight: '4px' }}>
        <Checkbox
      ...
      <FlexboxGrid.Item colspan={7} style={{ paddingRight: '4px' }}>
        <Text
          as="h3"
          truncate={true}
          data-testid="crew-member-agent"
          color={isAbsentFullMission ? THEME.color.slateGray : THEME.color.charcoal}
          fontStyle={isAbsentFullMission ? 'italic' : 'normal'}
          weight={isAbsentFullMission ? 'normal' : 'medium'}
          style={{ paddingLeft: '4px' }}
        >{`${crewMember?.agent ? [crewMember?.agent?.firstName, crewMember?.agent?.lastName].join(' ') : crewMember?.fullName}`}</Text>
      </FlexboxGrid.Item>

      <FlexboxGrid.Item colspan={6} style={{ paddingRight: '4px' }}>
        {
          <Text
            as="h3"
            truncate={true}
            data-testid="crew-member-role"
            color={isAbsentFullMission ? THEME.color.slateGray : THEME.color.charcoal}
            fontStyle={isAbsentFullMission ? 'italic' : 'normal'}
            weight={isAbsentFullMission ? 'normal' : 'medium'}
          >
            ...
            <IconButton
              ...
              title={
                isAbsentFullMission && MissionCrewAbsenceType.FULL_MISSION
                  ...
                  : 'Renseigner des absences temporaires'
              }
...
const InvitedCrewMember: React.FC<CrewListProps> = ({ index, name, crewMember, handleEditCrew, handleDelete }) => {
  return (
    <FlexboxGrid key={`${name}-${index}`} align={'middle'}>
      <FlexboxGrid.Item colspan={1}>
        <Icon.Account color={THEME.color.charcoal} />
      </FlexboxGrid.Item>
      <FlexboxGrid.Item colspan={7}>
        {
          <MissionCrewMemberText
            agent={crewMember?.agent}
