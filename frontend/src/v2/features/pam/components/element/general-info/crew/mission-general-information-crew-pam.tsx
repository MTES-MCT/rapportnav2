...
import MissionGeneralInformationCrewPamAbsenceModal from './mission-general-information-crew-pam-absence-modal.tsx'
import MissionGeneralInformationCrewPamForm from './mission-general-information-crew-pam-form.tsx'
import { useCrewAbsence } from '../../../../hooks/use-crew-absence.tsx'
...
const MissionGeneralInformationCrewPam: React.FC<MissionGeneralInformationCrewPamProps> = ({
  ...
  const { countFullMissionAbsences, getFullMissionAbsenceText } = useCrewAbsence()
  ...
  return (
    <>
      <MissionCrewUnderlineStack>
        <MissionCrewCollapsibleTitle onClick={() => setIsExpanded(!isExpanded)}>
          <Stack direction="row" alignItems="flex-start" spacing={'0.5rem'}>
            ...
            <Stack.Item>
              {!isExpanded && countFullMissionAbsences(currentCrewList) > 0 && (
                <Text as={'h3'}>• {getFullMissionAbsenceText(currentCrewList)}</Text>
              )}
            </Stack.Item>
          </Stack>
        </MissionCrewCollapsibleTitle>
      ...
      {selectedCrewIndex !== undefined && (
        <MissionGeneralInformationCrewPamAbsenceModal
          missionId={missionId}
