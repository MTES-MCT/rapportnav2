...
describe('MissionGeneralInformationCrewPam', () => {
  ...
  it('opens passenger form when adding a new member', async () => {
    ...
    const button = screen.getByText('Ajouter un passager')
    fireEvent.click(button)

    // Instead of looking for data-testid, look for the submit button which should be in the form
    await waitFor(() => {
      expect(screen.getByTestId('submit-passenger-form-button')).toBeInTheDocument()
    })
  })
  ...
  it('opens passenger form when editing an existing member', async () => {
    ...
    await waitFor(() => {
      // Check if submit button (which is part of the form) appears
      expect(screen.getByTestId('submit-passenger-form-button')).toBeInTheDocument()
    })
  ...
  it('handles deleting a crew member', () => {
  ...
  it('closes crew form when cancel is clicked', async () => {
    ...
    // Wait for submit button to appear (form is open)
    await waitFor(() => {
      expect(screen.getByTestId('submit-passenger-form-button')).toBeInTheDocument()
    })
    ...
    // Submit button should no longer be in the document (form is closed)
    await waitFor(() => {
      expect(screen.queryByTestId('submit-passenger-form-button')).not.toBeInTheDocument()
    })
  })
