...
// Mock the useCrewAbsenceForm hook
vi.mock('../../../../../hooks/use-crew-absence-form.tsx', () => ({
  useCrewAbsenceForm: () => ({
    initValue: {
      id: undefined,
      startDate: undefined,
...
describe('CrewAbsenceForm', () => {
  ...
  it('renders crew member name and role', () => {
    renderComponent()
    expect(screen.getByText('Jean Dupont')).toBeInTheDocument()
    expect(screen.getByText('Commandant')).toBeInTheDocument()
  })

  it('renders fullName when agent is not set', () => {
    const crewWithoutAgent = { id: '2', fullName: 'Marie Martin', role: { id: 'observer', title: 'Observateur' }, absences: [] }
    renderComponent({ crew: crewWithoutAgent }, { crew: [crewWithoutAgent] })
    expect(screen.getByText('Marie Martin')).toBeInTheDocument()
    expect(screen.getByText('Observateur')).toBeInTheDocument()
  })

  it('renders dash when role is not set', () => {
    const crewWithoutRole = { id: '3', agent: { id: 2, firstName: 'Paul', lastName: 'Blanc', services: [] }, absences: [] }
    renderComponent({ crew: crewWithoutRole }, { crew: [crewWithoutRole] })
    expect(screen.getByText('Paul Blanc')).toBeInTheDocument()
  ...
  describe('Temporary absence mode', () => {
    ...
    it('disables add button when form is not valid', () => {
      renderComponent({ absenceType: MissionCrewAbsenceType.TEMPORARY })

      const addButton = screen.getByText('Ajouter une absence temporaire').closest('button')
      // Button should be disabled when initial form is not valid (empty reason/dates)
      expect(addButton).toBeDisabled()

      // Clicking disabled button should not add new rows
      fireEvent.click(addButton!)
      const motifLabels = screen.getAllByText('Motif')
      expect(motifLabels.length).toBe(1)
    })
