...
vi.mock('../../../../../hooks/use-crew-absence.tsx', () => ({
  useCrewAbsence: () => ({
    countFullMissionAbsences: vi.fn(() => 0),
    getFullMissionAbsenceText: vi.fn(() => '0 membre non embarqué')
  })
}))
...
describe('MissionGeneralInformationCrewPam', () => {
  ...
  describe('collapsible functionality', () => {
    ...
    it('hides content when title is clicked', () => {
      ...
      const title = screen.getByText('Équipage')
      fireEvent.click(title)

      // Content should be hidden
      expect(screen.queryByText('John Doe')).not.toBeInTheDocument()
      expect(screen.queryByText("Ajouter un membre d'équipage")).not.toBeInTheDocument()
    })
    ...
    it('hides empty state message when collapsed', () => {
      ...
      expect(screen.queryByText("Sélectionner votre bordée, pour importer votre liste d'équipage")).not.toBeInTheDocument()
    })
  })
