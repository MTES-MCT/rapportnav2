...
export const simpleDateRangeValidationSchema = (opts: { allowSameDate?: boolean } = {}) => {
  const { allowSameDate = false } = opts
  ...
  return Yup.object().shape({
    ...
    endDateTimeUtc: Yup.date()
      ...
      .test('is-after-start', `La date et l'heure de fin doit être antérieure à la date de début`, function (value) {
        if (allowSameDate) return true

        const { startDateTimeUtc } = this.parent
        if (!startDateTimeUtc || !value) return true

        return new Date(value) > new Date(startDateTimeUtc)
      })
  })
}
