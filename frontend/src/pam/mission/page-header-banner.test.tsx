...
describe('MissionPageHeaderBanner', () => {
    ...
    test('should render the text incomplete when only missing data from unit', () => {
        ...
        render(<MissionPageHeaderBanner reportStatus={reportStatus}/>);
        const element = screen.getByText('La mission est terminée mais le rapport n\'est pas complet : des données sont manquantes dans des champs à remplir par votre unité.')
        expect(element).toBeInTheDocument()
    ...
    test('should render the text incomplete when missing data from several sources including the unit', () => {
        const reportStatus = {status: MissionReportStatusEnum.INCOMPLETE, source: [MissionSourceEnum.MONITORFISH, MissionSourceEnum.RAPPORTNAV]}
        render(<MissionPageHeaderBanner reportStatus={reportStatus}/>);
        const element = screen.getByText('La mission est terminée mais le rapport n\'est pas complet : des données sont manquantes dans des champs à remplir par votre unité.')
        expect(element).toBeInTheDocument()
        expect(getComputedStyle(element).color).toBe(hexToRgb(THEME.color.maximumRed))
    ...
    test('should render the text incomplete when missing data from CACEM', () => {
        const reportStatus = {status: MissionReportStatusEnum.INCOMPLETE, source: [MissionSourceEnum.MONITORENV]}
        render(<MissionPageHeaderBanner reportStatus={reportStatus}/>);
        const element = screen.getByText('La mission est terminée mais le rapport n\'est pas complet : des données sont manquantes dans des champs à remplir par le CACEM.')
        expect(element).toBeInTheDocument()
    ...
    test('should render the text incomplete when missing data from CNSP', () => {
        const reportStatus = {status: MissionReportStatusEnum.INCOMPLETE, source: [MissionSourceEnum.MONITORFISH]}
        render(<MissionPageHeaderBanner reportStatus={reportStatus}/>);
        const element = screen.getByText('La mission est terminée mais le rapport n\'est pas complet : des données sont manquantes dans des champs à remplir par le CNSP.')
        expect(element).toBeInTheDocument()
        expect(getComputedStyle(element).color).toBe(hexToRgb(THEME.color.charcoal))
    });
});
