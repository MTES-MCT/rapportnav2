...
describe('StatusReasonDropdown', () => {
    test('renders not render the dropdown for NAVIGATING', () => {
        const {container} = render(<StatusReasonDropdown actionType={ActionStatusType.NAVIGATING} value={undefined}
                                                         onSelect={vi.fn()}/>);
        expect(container.firstChild).toBeNull();
    });
    test('renders not render the dropdown for ANCHORED', () => {
        const {container} = render(<StatusReasonDropdown actionType={ActionStatusType.ANCHORED} value={undefined}
                                                         onSelect={vi.fn()}/>);
        expect(container.firstChild).toBeNull();
    });
    test('renders not render the dropdown for UNKNOWN', () => {
        const {container} = render(<StatusReasonDropdown actionType={ActionStatusType.UNKNOWN} value={undefined}
                                                         onSelect={vi.fn()}/>);
        expect(container.firstChild).toBeNull();
    });
