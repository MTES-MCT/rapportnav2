...
class APIEnvMissionRepositoryTest {
    ...
    @Nested
    inner class FindAllControlPlans {
        ...
        @Test
        fun `should fetch control plans and return entity`() {
            val realMapper = createRealMapper()
            val controlPlansJson = """
                {
                    "themes": {
                        "1": {"id": 1, "theme": "Police des mouillages"}
                    },
                    "subThemes": {
                        "10": {"id": 10, "themeId": 1, "subTheme": "ZMEL", "year": 2024}
                    },
                    "tags": {
                        "100": {"id": 100, "tag": "Urgent", "themeId": 1}
                    }
                }
            """.trimIndent()
            ...
            Mockito.`when`(httpClientFactory.create()).thenReturn(httpClient)
            Mockito.`when`(httpResponse.statusCode()).thenReturn(200)
            Mockito.`when`(httpResponse.body()).thenReturn(controlPlansJson)
            Mockito.`when`(
                httpClient.send(
                    Mockito.any(HttpRequest::class.java),
                    Mockito.any<HttpResponse.BodyHandler<String>>()
            ...
            val result = envRepo.findAllControlPlans()

            assertNotNull(result)
            assertEquals(1, result?.themes?.size)
            assertEquals(1, result?.subThemes?.size)
            assertEquals(1, result?.tags?.size)
        }

        @Test
        fun `should call correct URL for control plans`() {
            val realMapper = createRealMapper()
            val controlPlansJson = """{"themes": {}, "subThemes": {}, "tags": {}}"""
            ...
            envRepo.findAllControlPlans()

            verify(httpClient).send(
                argThat { request ->
                    request.uri() == URI.create("$host/api/v1/control_plans")
                },
                Mockito.any<HttpResponse.BodyHandler<String>>()
            )
        }
        ...
        @Test
        fun `should return null when API throws exception`() {
            val realMapper = createRealMapper()
        ...
        @Test
        fun `should handle empty control plans`() {
            val realMapper = createRealMapper()
            val controlPlansJson = """{"themes": {}, "subThemes": {}, "tags": {}}"""
            ...
            assertNotNull(result)
            assertTrue(result?.themes?.isEmpty() ?: false)
            assertTrue(result?.subThemes?.isEmpty() ?: false)
            assertTrue(result?.tags?.isEmpty() ?: false)
        }
    }
}
