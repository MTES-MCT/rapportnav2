...
import fr.gouv.dgampa.rapportnav.domain.use_cases.mission.v2.GetCrew
...
@SpringBootTest(classes = [GetCrew::class])
class GetCrewTest {
    @Autowired
    ...
    fun `execute get service, empty list if new service id is null`() {
        val serviceId = 3
        val result = getCrew.execute(newServiceId = null)
    ...
    fun `execute get service, in database when new service id and old service id are different`() {
        ...
        val result = getCrew.execute(newServiceId = serviceId, oldServiceId = 2)
    ...
    fun `execute get service, in general infos when new serviceId and old serviceId are the same`() {
        ...
        val generalInfos = MissionGeneralInfo(
            crew = crew.map { MissionCrew.fromMissionCrewEntity(it) }
        )
        val result = getCrew.execute(
