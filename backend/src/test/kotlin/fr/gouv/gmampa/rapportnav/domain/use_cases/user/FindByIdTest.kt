...
class FindByIdTest {
    ...
    fun `should throw BackendInternalException when repository fails`() {
        ...
        `when`(userRepository.findById(userId)).thenAnswer { throw RuntimeException("Database error") }

        val exception = assertThrows<BackendInternalException> {
            findById.execute(userId)
        }
        assertThat(exception.message).contains("FindById failed for userId=$userId")
    }
