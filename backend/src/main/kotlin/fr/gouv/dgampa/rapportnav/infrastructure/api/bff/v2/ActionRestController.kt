...
class ActionRestController(
    ...
    private val getActionByOwnerId: GetActionByOwnerId,
    ...
    fun getActions(@PathVariable(name = "ownerId") ownerId: String): List<MissionAction?> {
        val missionUUID = if (isValidUUID(ownerId)) {
            UUID.fromString(ownerId)
        } else {
            val extId = Integer.valueOf(ownerId)
            missionNavRepository.findByExternalId(extId.toString()).orElse(null)?.id
                ?: return emptyList()
        }
    ...
    fun getActionById(
        ...
        return MissionAction.fromMissionActionEntity(
            getActionByOwnerId.execute(ownerId = ownerId, actionId = actionId)
        )
    }
