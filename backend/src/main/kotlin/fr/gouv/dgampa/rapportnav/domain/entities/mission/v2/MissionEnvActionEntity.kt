...
data class MissionEnvActionEntity(
    ...
    override fun computeValidity(validator: EntityValidityValidator, missionDates: MissionDates?) {
        ...
        if (!monitorEnvComplete) {
            sourcesOfMissingData.add(MissionSourceEnum.MONITORENV)
        }
        ...
        val overallStatus = when {
            rapportNavComplete && monitorEnvComplete -> CompletenessForStatsStatusEnum.VALID
            !monitorEnvComplete || !this.controlsToComplete.isNullOrEmpty() -> CompletenessForStatsStatusEnum.INCOMPLETE
